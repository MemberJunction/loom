...
| **Phase 02.7: Multi-Cycle Child Row Generation from Motifs & Ladders** | Deferred to Plan 03 | Generate discrete child rows per cycle from `motif.childRates` and ladder bindings (`childRates[].fixedFields`). Enables child-aggregation feature pins across multi-cycle history. | Integration test verifying child row counts match rates per cycle. | Carried forward to Plan 03 |
| **Phase 02.8: Single Cycle Unit & Manifest Typed Fields** | N6, R5-3, R5-5 | Single `cycleUnit` declared in `ProjectManifestSchema` (`year`, `week`, `month`, `cycle`) and used consistently by `build` and `accumulate` (N6, R5-5). Ladders advance only when whole cycle elapsed; `cyclesSinceBirth` calculated from `birthCycle`. `startCycle` and `releaseDate` typed required fields in manifest; all `as Record<string, unknown>` casts eliminated (R5-3). Dynamic cycle units and derived transition counting (R7-3, R7-4) carried forward. | `integration-enterprise.test.ts`: 12-cycle test asserts 5 status transitions matching `durationCycles` prediction; `startCycle: 2015` unrolls from 2015. | `MemberJunction/loom#6` |

---

### 8. Carried Forward from Plan 02 (Scheduled for Plan 03)

| Item | Description | Verification Target |
|---|---|---|
| **R7-1** | Adding a hero must not change non-hero rows (identity-keyed generation, CLI Invariant 3). | Integration test: generating with vs without a hero produces identical non-hero rows. |
| **R7-2** | Remove the forced hero outcome override at emission. | Emitter test: hero outcomes follow calibrated model without synthetic post-hoc overwrite. |
| **02.7** | Child rows per cycle: Generate discrete child rows per cycle from `motif.childRates` and ladder bindings (`childRates[].fixedFields`). | Integration test: child row counts match rates per cycle across multi-cycle history. |
| **N6** | State ladder `stateValues` scope binding. | Schema test: ladders enforce typed `stateValues` matching domain scope. |
| **R7-3 & R7-4** | Dynamic cycle units (`year`/`week`/`month`) and derived transition counting. | CLI test: non-hardcoded cycle advancement with dynamically computed transition counts. |
